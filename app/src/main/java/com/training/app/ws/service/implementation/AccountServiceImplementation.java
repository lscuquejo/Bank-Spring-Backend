...
public class AccountServiceImplementation implements AccountService {
    ...
    public AccountDto createAccount(AccountDto account) {
        ...
        } throw new AccountServiceExcepetion(ErrorMessages.BAD_REQUEST.getErrorMessage() + "name field : required field is required");
    }
    ...
    public AccountDto updateAccount(String uId, AccountDto account) {
        if (account.getName() != null) {
            ...
            List<String> splitedNameAsList = Arrays.asList(splitedName);
            if (splitedNameAsList.size() > 1) {
                if(accountEntity == null) throw new AccountServiceExcepetion(ErrorMessages.NO_RECORD_FOUND.getErrorMessage());
